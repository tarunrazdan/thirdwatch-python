...
class AddToCartApi(object):
    ...
    def add_to_cart(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.add_to_cart(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param AddToCart json: Pass added item info to thirdwatch. Only `_userID` is required field. But this should contain item info. (required)
        ...
        if kwargs.get('callback'):
            return self.add_to_cart_with_http_info(json, **kwargs)
        else:
            (data) = self.add_to_cart_with_http_info(json, **kwargs)
            return data

    def add_to_cart_with_http_info(self, json, **kwargs):
        """
        Use add_to_cart when a user adds an item to their shopping cart or list.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.add_to_cart_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method add_to_cart" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `add_to_cart`")


        collection_formats = {}

        resource_path = '/v1/add_to_cart'.replace('{format}', 'json')
        path_params = {}
