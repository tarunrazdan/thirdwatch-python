...
class CreateAccountApi(object):
    ...
    def create_account(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_account(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param CreateAccount json: Pass user details after registration. Only `_userID` is required field. But this should contain user info. (required)
        ...
        if kwargs.get('callback'):
            return self.create_account_with_http_info(json, **kwargs)
        else:
            (data) = self.create_account_with_http_info(json, **kwargs)
            return data

    def create_account_with_http_info(self, json, **kwargs):
        """
        Use create_account to pass user details at user registration.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_account_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `create_account`")


        collection_formats = {}

        resource_path = '/v1/create_account'.replace('{format}', 'json')
        path_params = {}
