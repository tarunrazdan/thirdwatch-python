...
class UpdateOrderApi(object):
    ...
    def update_order(self, json, **kwargs):
        """
        ...
        * This event contains the same fields as ```create_order```. * The existing order will be completely replaced by the values sent in `update_order`. Be sure to specify all values for the order, not just those that changed. * If no matching `_orderId` found, a new order will be created. 
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.update_order(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param UpdateOrder json: Update details of an existing order. Only `_userID` is required field. But this should contain existing order info. (required)
        ...
        if kwargs.get('callback'):
            return self.update_order_with_http_info(json, **kwargs)
        else:
            (data) = self.update_order_with_http_info(json, **kwargs)
            return data

    def update_order_with_http_info(self, json, **kwargs):
        """
        Update details of an existing order.
        * This event contains the same fields as ```create_order```. * The existing order will be completely replaced by the values sent in `update_order`. Be sure to specify all values for the order, not just those that changed. * If no matching `_orderId` found, a new order will be created. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.update_order_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `update_order`")


        collection_formats = {}

        resource_path = '/v1/update_order'.replace('{format}', 'json')
        path_params = {}
