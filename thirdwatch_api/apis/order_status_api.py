...
class OrderStatusApi(object):
    ...
    def order_status(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.order_status(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param OrderStatus json: Pass order status to thirdwatch. Only `_userID` is required field. But this should contain order info. (required)
        ...
        if kwargs.get('callback'):
            return self.order_status_with_http_info(json, **kwargs)
        else:
            (data) = self.order_status_with_http_info(json, **kwargs)
            return data

    def order_status_with_http_info(self, json, **kwargs):
        """
        Use order_status to track the order processing workflow of a previously submitted order.
        For example, order_status can be used to indicate that an order has been held for review, canceled due to suspected fraud, or fulfilled. This event can be called multiple times to record changes an order's status. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.order_status_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `order_status`")


        collection_formats = {}

        resource_path = '/v1/order_status'.replace('{format}', 'json')
        path_params = {}
