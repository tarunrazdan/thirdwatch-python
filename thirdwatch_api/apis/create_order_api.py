...
class CreateOrderApi(object):
    ...
    def create_order(self, body, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_order(body, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param CreateOrder body: An order to submit for review. Only `_userID` is required field. But this should contain order info. (required)
        ...
        if kwargs.get('callback'):
            return self.create_order_with_http_info(body, **kwargs)
        else:
            (data) = self.create_order_with_http_info(body, **kwargs)
            return data

    def create_order_with_http_info(self, body, **kwargs):
        """
        Submit a new or existing order to Thirdwatch for review. This API should contain order item info, the payment info, and user identity info.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_order_with_http_info(body, callback=callback_function)
        ...
        resource_path = '/v1/createOrder'.replace('{format}', 'json')
        path_params = {}
