...
class LogoutApi(object):
    ...
    def logout(self, json, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.logout(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param Logout json: Pass logout status to thirdwatch. Only `_userID` is required field. But this should contain logout info. (required)
        ...
        if kwargs.get('callback'):
            return self.logout_with_http_info(json, **kwargs)
        else:
            (data) = self.logout_with_http_info(json, **kwargs)
            return data

    def logout_with_http_info(self, json, **kwargs):
        """
        Use logout to record when a user logs out.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.logout_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `logout`")


        collection_formats = {}

        resource_path = '/v1/logout'.replace('{format}', 'json')
        path_params = {}
