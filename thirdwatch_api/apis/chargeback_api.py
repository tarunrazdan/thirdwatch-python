...
class ChargebackApi(object):
    ...
    def chargeback(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.chargeback(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param Chargeback json: Pass chargeback to thirdwatch. Only `_userID` is required field. But this should contain chargeback info. (required)
        ...
        if kwargs.get('callback'):
            return self.chargeback_with_http_info(json, **kwargs)
        else:
            (data) = self.chargeback_with_http_info(json, **kwargs)
            return data

    def chargeback_with_http_info(self, json, **kwargs):
        """
        Use chargeback to capture a chargeback reported on a transaction. This event can be called multiple times to record changes to the chargeback state.
        Note - When you send a chargeback event you also need to send a label event if you want to prevent the user from making another purchase. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.chargeback_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method chargeback" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `chargeback`")


        collection_formats = {}

        resource_path = '/v1/chargeback'.replace('{format}', 'json')
        path_params = {}
