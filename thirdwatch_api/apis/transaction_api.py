...
class TransactionApi(object):
    ...
    def transaction(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.transaction(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param Transaction json: Pass transaction results to thirdwatch. Only `_userID` is required field. But this should contain transaction info. (required)
        ...
        if kwargs.get('callback'):
            return self.transaction_with_http_info(json, **kwargs)
        else:
            (data) = self.transaction_with_http_info(json, **kwargs)
            return data

    def transaction_with_http_info(self, json, **kwargs):
        """
        Use transaction to record attempts results to Pay, Transfer money, Refund or other transactions.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.transaction_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `transaction`")
