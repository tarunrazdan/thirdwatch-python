...
class LinkSessionToUserApi(object):
    """
    ...
    def link_session_to_user(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.link_session_to_user(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param LinkSessionToUser json: Pass session and user to thirdwatch for link. Only `_userID` is required field. But this should contain session and user info. (required)
        ...
        if kwargs.get('callback'):
            return self.link_session_to_user_with_http_info(json, **kwargs)
        else:
            (data) = self.link_session_to_user_with_http_info(json, **kwargs)
            return data

    def link_session_to_user_with_http_info(self, json, **kwargs):
        """
        Use link_session_to_user to associate specific session to a user. Generally used only in anonymous checkout workflows.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.link_session_to_user_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method link_session_to_user" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `link_session_to_user`")


        collection_formats = {}

        resource_path = '/v1/link_session_to_user'.replace('{format}', 'json')
        path_params = {}
