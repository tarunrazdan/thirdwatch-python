...
class SubmitReviewApi(object):
    ...
    def submit_review(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.submit_review(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param SubmitReview json: Pass review to thirdwatch. Only `_userID` is required field. But this should contain review info. (required)
        ...
        if kwargs.get('callback'):
            return self.submit_review_with_http_info(json, **kwargs)
        else:
            (data) = self.submit_review_with_http_info(json, **kwargs)
            return data

    def submit_review_with_http_info(self, json, **kwargs):
        """
        Use submit_review when a user-submitted review of a product or seller.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.submit_review_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method submit_review" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `submit_review`")


        collection_formats = {}

        resource_path = '/v1/submit_review'.replace('{format}', 'json')
        path_params = {}
