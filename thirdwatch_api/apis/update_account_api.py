...
class UpdateAccountApi(object):
    ...
    def update_account(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.update_account(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param UpdateAccount json: Pass user details after update or change in user info. Only `_userID` is required field. But this should contain user info. (required)
        ...
        if kwargs.get('callback'):
            return self.update_account_with_http_info(json, **kwargs)
        else:
            (data) = self.update_account_with_http_info(json, **kwargs)
            return data

    def update_account_with_http_info(self, json, **kwargs):
        """
        Use update_account to record changes to the user's account information.
        For user accounts created before integration with Thirdwatch, there's no need to call `create_account` before `update_account`. Simply call `update_account` and we'll infer that account was created before integration. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.update_account_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `update_account`")


        collection_formats = {}

        resource_path = '/v1/update_account'.replace('{format}', 'json')
        path_params = {}
