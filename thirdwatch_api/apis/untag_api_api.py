...
class UntagAPIApi(object):
    ...
    def un_tag_user(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.un_tag_user(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param UnTag json: Pass user and it's info to thirdwatch. Only `_userID` is required field. But this should contain untag info. (required)
        ...
        if kwargs.get('callback'):
            return self.un_tag_user_with_http_info(json, **kwargs)
        else:
            (data) = self.un_tag_user_with_http_info(json, **kwargs)
            return data

    def un_tag_user_with_http_info(self, json, **kwargs):
        """
        If you are unsure whether a user is bad or good, you can always remove tag and leave the user in a neutral state.
        To untag a user for a particular abuse type, send the abuse_type key in json data. In the rare case that you want to remove all tags for all abuse types for a particular user, send without the abuse_type query parameter. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.un_tag_user_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method un_tag_user" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `un_tag_user`")


        collection_formats = {}

        resource_path = '/v1/untag'.replace('{format}', 'json')
        path_params = {}
