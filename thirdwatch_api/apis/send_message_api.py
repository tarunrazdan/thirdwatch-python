...
class SendMessageApi(object):
    ...
    def send_message(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.send_message(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param SendMessage json: Pass message to thirdwatch. Only `_userID` is required field. But this should contain message info. (required)
        ...
        if kwargs.get('callback'):
            return self.send_message_with_http_info(json, **kwargs)
        else:
            (data) = self.send_message_with_http_info(json, **kwargs)
            return data

    def send_message_with_http_info(self, json, **kwargs):
        """
        Use send_message to record when a user sends a message to other i.e. seller, support.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.send_message_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `send_message`")


        collection_formats = {}

        resource_path = '/v1/send_message'.replace('{format}', 'json')
        path_params = {}
