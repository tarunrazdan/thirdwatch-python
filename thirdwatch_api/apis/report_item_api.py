...
class ReportItemApi(object):
    ...
    def report_item(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.report_item(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param ReportItem json: Pass report item info to thirdwatch. Only `_userID` is required field. But this should contain item id. (required)
        ...
        if kwargs.get('callback'):
            return self.report_item_with_http_info(json, **kwargs)
        else:
            (data) = self.report_item_with_http_info(json, **kwargs)
            return data

    def report_item_with_http_info(self, json, **kwargs):
        """
        Use report_item to let us know when another user reports that this item may violate your company’s policies.
        If you have a feature like \"Report Item\" or \"Flag this Item\", send that event to us using this. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.report_item_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method report_item" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `report_item`")


        collection_formats = {}

        resource_path = '/v1/report_item'.replace('{format}', 'json')
        path_params = {}
