...
class CustomEventApi(object):
    ...
    def custom_event(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.custom_event(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param Custom json: Pass order status to thirdwatch. Only `_userID` is required field. But this should contain custom info. (required)
        ...
        if kwargs.get('callback'):
            return self.custom_event_with_http_info(json, **kwargs)
        else:
            (data) = self.custom_event_with_http_info(json, **kwargs)
            return data

    def custom_event_with_http_info(self, json, **kwargs):
        """
        Use order_status to track the order processing workflow of a previously submitted order.
        Custom events and fields capture user behavior and differences not covered by our reserved events and fields. For example, a location tracking business can create a custom event called trackLocation with custom fields that are relevant. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.custom_event_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method custom_event" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `custom_event`")


        collection_formats = {}

        resource_path = '/v1/custom_event'.replace('{format}', 'json')
        path_params = {}
