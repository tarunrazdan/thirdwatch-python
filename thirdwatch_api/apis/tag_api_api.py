...
class TagAPIApi(object):
    ...
    def tag_user(self, json, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.tag_user(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param Tag json: Pass user and it's info to thirdwatch. Only `_userID` is required field. But this should contain tag info. (required)
        ...
        if kwargs.get('callback'):
            return self.tag_user_with_http_info(json, **kwargs)
        else:
            (data) = self.tag_user_with_http_info(json, **kwargs)
            return data

    def tag_user_with_http_info(self, json, **kwargs):
        """
        The Tag API enables you to tell Thirdwatch which of your users are bad and which are good.
        By telling us who is bad and who is good, we can identify patterns that are unique to your business. Once you give this feedback, the platform instantly analyzes it and learns to identify good and bad behavior of other users more accurately. If you want to change an existing label for a user - for example, from bad to good - all you need to do is send a new label and we'll overwrite the existing value. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.tag_user_with_http_info(json, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method tag_user" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'json' is set
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `tag_user`")


        collection_formats = {}

        resource_path = '/v1/tag'.replace('{format}', 'json')
        path_params = {}
