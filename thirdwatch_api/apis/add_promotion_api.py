...
class AddPromotionApi(object):
    ...
    def add_promotion(self, json, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.add_promotion(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param AddPromotion json: Pass added promotion info to thirdwatch. Only `_userID` is required field. But this should contain promotion info. (required)
        ...
        if kwargs.get('callback'):
            return self.add_promotion_with_http_info(json, **kwargs)
        else:
            (data) = self.add_promotion_with_http_info(json, **kwargs)
            return data

    def add_promotion_with_http_info(self, json, **kwargs):
        """
        Use add_promotion to record when a user adds one or more promotions to their account.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.add_promotion_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `add_promotion`")


        collection_formats = {}

        resource_path = '/v1/add_promotion'.replace('{format}', 'json')
        path_params = {}
