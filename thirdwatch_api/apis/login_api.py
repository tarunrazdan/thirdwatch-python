...
class LoginApi(object):
    ...
    def login(self, json, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.login(json, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param Login json: Pass login status to thirdwatch. Only `_userID` is required field. But this should contain login info. (required)
        ...
        if kwargs.get('callback'):
            return self.login_with_http_info(json, **kwargs)
        else:
            (data) = self.login_with_http_info(json, **kwargs)
            return data

    def login_with_http_info(self, json, **kwargs):
        """
        Use login to record when a user attempts to log in.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.login_with_http_info(json, callback=callback_function)
        ...
        if ('json' not in params) or (params['json'] is None):
            raise ValueError("Missing the required parameter `json` when calling `login`")
